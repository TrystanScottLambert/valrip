...
def _compare_column_type(
    column_name: str, data: pl.DataFrame, metadata_columns: dict[str, ColumnMetaData]
) -> Status:
    """
    Compares the information for the given column name in both the data and the metadata.
    """
    if check_data_type(
        data[column_name].dtype, metadata_columns[column_name].data_type
    ):
        return Status(State.PASS)
    else:
        return Status(
            State.FAIL,
            f"Invalid datatype. MAML indicates the expected type is {metadata_columns[column_name].data_type} but the Parquet shows type actually is {data[column_name].dtype}",
        )
    ...
    def __post_init__(self) -> None:
        self.valid = all(
            [
                ...
                and self.valid_column_datatypes.state == State.PASS,
                self.valid_column_ranges
                and self.valid_column_ranges.state == State.PASS,
            ]
        )
...
def validate_data_and_metadata(
    table_name: str, data: pl.DataFrame, metadata: MamlMetaData
) -> list[DataMetadataValueReport]:
    """
    ...
    for column_name in all_columns:
        ...
        if column_name in data.columns and column_name in metadata_columns.columns:
            ...
            column_name in data.columns and column_name not in metadata_columns.columns
        ):
            ...
            column_name not in data.columns and column_name in metadata_columns.columns
        ):
            in_both_files = Status(
                State.FAIL,
